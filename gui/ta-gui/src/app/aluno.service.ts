...
export class AlunoService {
  ...
  remover(aluno: Aluno): Promise<Aluno> {
    return this.http.delete(this.taURL + "/aluno/" + aluno.cpf,{headers : this.headers})
    .toPromise()
    .then(res => {
       if (res.json().success) {return aluno;} else {return null;}
    })
  ...
  checarCPF(aluno: Aluno): Promise<Aluno> {
    return this.http.get(this.taURL+"/checkCPF",JSON.stringify(aluno)).toPromise().then(res => res.json() as Aluno).catch(this.tratarErro);
  }
